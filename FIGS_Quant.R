...
r_run = function(mzmlPath, CoeffsPath, fdrValue=0.02){
    library(kza)
    library(pracma)
    #library(zoo)
    library(moments)
    ...
    library(MASS)
    source('FIGS_Quant.R')
    headerPath = paste0(mzmlPath, "/header.csv")
    resultsPath = paste0(CoeffsPath, "/Coeffs.csv")
    decoyResultsPath = paste0(CoeffsPath, "/DecoyCoeffs.csv")
    cat("start:", headerPath,"\n", file="log.txt",append=T)
    ...
    cutoff = min(D$Score[which(fdr < fdrValue)],na.rm=TRUE)
    ...
    cat("end: file save to",outputPath,"\n", file="log.txt",append=T)
}
